...
impl<T> NetworkNode<T> {
    ...
    pub fn decode(&self) -> Result<T, mpsc::RecvError> {
    ...
    /// Receives a message from the channel before the given timeout.
    pub fn decode_timeout(&self, timeout: Instant) -> Result<T, mpsc::RecvTimeoutError> {
        match self.channel.1.try_recv() {
            Ok(msg) => Ok(msg),
            Err(_) => {
                let now = Instant::now();
